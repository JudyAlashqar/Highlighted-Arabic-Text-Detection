...
def draw_text_image(
    ...
    character_font_pairs = [
        (
            character,
            next(
                font for subalphabet, font in fonts.items() if seperate_character in subalphabet
            ),
        )
        for character, seperate_character in zip(text, original_text)
    ]
    ...
    if lang != 'ar':
        ...
        for character_index, (character, font) in enumerate(character_font_pairs):
            ...
            if out_of_space:
                break
            max_y = max(y + character_height + offset_y, max_y)
            draw.text(xy=(x - dx, y), text=character, fill=color + (255,), font=font)
            for subcharacter in subcharacters:
                lines_raw[-1].append(
                    (
                        np.array(
